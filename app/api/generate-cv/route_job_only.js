...
export async function POST(request) {
  try {
    ...
    if (generationType === 'cv_from_job') {
      // Generate CV from job offer only
      if (!jobDoc) {
        return NextResponse.json({
          error: 'Aucune offre d\'emploi trouvée. Veuillez uploader une offre d\'emploi pour générer un CV sur mesure.'
        }, { status: 400 })
      }

      console.log('Generating CV from job offer only')
      ...
      if (!jobText || jobText.length < 10) {
        return NextResponse.json({
          error: 'Le contenu de l\'offre d\'emploi est insuffisant pour générer un CV.'
        }, { status: 400 })
      }
...
RÉPONDS UNIQUEMENT avec le CV au format JSON :
{
  ...
  "keywords": ["mot-clé 1", "mot-clé 2", "mot-clé 3"]
      ...
      console.log('Generating CV from job offer...')
...
- RÉPONDS UNIQUEMENT avec le texte de la lettre de motivation, rien d'autre.`
      ...
      console.log('CV and cover letter generation completed')
