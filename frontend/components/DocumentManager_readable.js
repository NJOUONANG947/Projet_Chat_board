  ...
  const generateCoverLetter = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        // Téléchargement manuel seulement - pas automatique
      }
    } catch (error) {
  ...
  const generateOptimizedCV = async () => {
    ...
    try {
      ...
      if (data.error) {
        ...
        // Téléchargement manuel seulement
      }
    } catch (error) {
    ...
    <div className="document-manager max-w-6xl mx-auto p-6">
      <div className="mb-8">
        <div className="flex justify-between items-center">
          <h1 className="text-3xl font-bold text-white">Gestionnaire de Documents</h1>
          <div className="flex space-x-2">
            <button
              onClick={onClose}
              className="px-4 py-2 bg-black/20 backdrop-blur-sm border border-white/10 text-white rounded-lg hover:bg-black/30 transition-colors"
            >
              Retour au Chat
            </button>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-1">
          ...
          >
            <h2 className="text-xl font-semibold text-white mb-4">Uploader un Document</h2>
            <div className="space-y-4">
              <div>
              ...
              {uploading && (
                <div className="text-center py-4">
                  ...
                  <p className="text-sm text-gray-300 mt-2">Upload en cours...</p>
                </div>
              )}
            </div>
          </motion.div>
        ...
        <div className="lg:col-span-1">
          <motion.div
            ...
            className="glass-card p-6"
          >
            <h2 className="text-xl font-semibold text-white mb-4">Mes Documents</h2>
            ...
            {loading ? (
              <div className="text-center py-8">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-400 mx-auto"></div>
                <p className="text-sm text-gray-300 mt-2">Chargement...</p>
              </div>
            ) : documents.length === 0 ? (
                  ...
                  <div
                    ...
                    className={`p-3 border rounded-lg cursor-pointer transition-all ${
                      selectedDocuments.includes(doc.id)
                        ...
                        : 'border-white/10 hover:border-white/20 bg-black/20 backdrop-blur-sm'
                    }`}
                  >
        ...
        <div className="lg:col-span-1">
          <motion.div
            ...
            className="glass-card p-6"
          >
            <h2 className="text-xl font-semibold text-white mb-4">Résultats</h2>
            {analyzing ? (
              <div className="text-center py-8">
                <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-blue-400 mx-auto"></div>
                <p className="text-sm text-gray-300 mt-2">Génération en cours...</p>
              </div>
            ) : generatedLetter ? (
              renderGeneratedLetter()
            ) : selectedDocuments.length > 0 ? (
              <p className="text-gray-300">Cliquez sur un bouton de génération pour créer du contenu personnalisé.</p>
            ) : (
              <p className="text-gray-300">Sélectionnez un ou plusieurs documents pour commencer.</p>
            )}
          </motion.div>
        </div>
      </div>
